...
export const getAdSpendRevenue = async () => {
    try {
        const response = await api.get('/api/marketing/ad-spend-revenue');
        return response.data;
    } catch (error) {
        console.error('Error fetching ad spend overview:', error);
        throw error;
    }
};

export const getCTRTrend = async () => {
    const response = await api.get('/api/marketing/ad-ctr-trend');
    return response.data;
};

export const getAdChannelConversions = async () => {
    const response = await api.get('/api/marketing/channel-conversions');
    return response.data;
};

export const getCustomerAcquisitionCost = async () => {
    const response = await api.get('/api/marketing/customer-acquisition-cost');
    return response.data;
};
